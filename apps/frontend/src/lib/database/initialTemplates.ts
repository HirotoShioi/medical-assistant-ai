...
処方内容の要約方法:
- それぞれの薬品について、調べた上で一般名に置換してください。
例: クレストール錠5mg -> ロスバスタチンカルシウム
- リストの先頭には何もつけないでください。("-"や"・"は使用しないでください)
- それぞれの処方内容は1行にまとめてください。
    {薬名} {1回当たりの投薬量} {1日の服用回数} {服用時間}
    <フォーマット例>
    ロスバスタチンカルシウム5mg 1回1錠 1日1回 朝食後
    ランソプラゾール15mg 1回1錠 1日1回 朝食後
    メトホルミン塩酸塩250mg 1回1錠 1日2回 朝夕食後
    ラコールNF配合経腸用液400mL 1回1包 1日3回 朝昼夕食後
    リナグリプチン5mg 1回1錠 1日1回 朝食後
    </フォーマット例>

- 処方内容以外の情報は含めないでください。
- 服用期間は含めないでください。
- 服用終了日の計算には細心の注意を払ってください。「09-27から28日分」とあれば、終了日は9月27日から28日後の10月24日となります。
- searchMedicineを使用して、薬品の情報を調べてください。検索し、薬品情報があった場合には、その薬品名、一般名、URLをテーブル形式で最後に記載してください。
例: 
## 詳細内容

| 薬品名 | 一般名 | URL |
| --- | --- | --- | --- | --- |
| クレストール | ロスバスタチンカルシウム | https://example.com/rosuvastatin-calcium |
| ランソプラゾール | ランソプラゾール | https://example.com/ranzoprazole | 
| メトホルミン | メトホルミン | https://example.com/metformin |
| トラゼンタ錠 | リナグリプチン | https://example.com/linagliptin |
...
`,
  ...
  また処方内容のフォーマットパターンを指定してください。

  パターン1. 
    {薬名} {1回当たりの投薬量} {1日の服用回数} {服用時間}
    
    例: ロスバスタチンカルシウム5mg 1回1錠 1日1回 朝食後

...
export async function search(query: string, options?: SearchOptions): Promise<SearchResults> {
	...
	if (text.includes('DDG.deep.is506')) throw new Error('A server error occurred!');
	if (text.includes('DDG.deep.anomalyDetectionBlock'))
		throw new Error('DDG detected an anomaly in the request, you are likely making requests too quickly.');
	const searchResults = JSON.parse(SEARCH_REGEX.exec(text)![1].replace(/\t/g, '    ')) as (CallbackSearchResult | CallbackNextSearch)[];
	...
	// Images
	const imagesMatch = IMAGES_REGEX.exec(text);
	if (imagesMatch) {
	...
	// News
	const newsMatch = NEWS_REGEX.exec(text);
	if (newsMatch) {
	...
	// Videos
	const videosMatch = VIDEOS_REGEX.exec(text);
	if (videosMatch) {
	...
	// Related Searches
	const relatedMatch = RELATED_SEARCHES_REGEX.exec(text);
	if (relatedMatch) {
...
export async function autocomplete(query: string, region?: string): Promise<AutocompleteResult[]> {
	...
	const response = await fetch(`https://duckduckgo.com/ac/?${queryString(queryObject)}`);
	const text = await response.text();
